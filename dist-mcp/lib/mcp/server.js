...
export class McpServer {
    ...
    async getSocialRepo(username) {
        try {
            return await this.githubClient.get(`/repos/${encodeURIComponent(username)}/${SOCIAL_REPO}`, { auth: false });
        }
        catch (err) {
            if (err?.status === 404)
                return null;
    ...
    async readFileOrNull(owner, repo, path, ref) {
        try {
            const data = await this.githubClient.get(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${encodeURIComponent(path)}?ref=${encodeURIComponent(ref)}`);
    ...
    async handleToolCall(toolName, args) {
        switch (toolName) {
            case 'get_profile': {
                ...
                const user = await this.githubClient.get(`/users/${encodeURIComponent(username)}`, { auth: false });
            ...
            case 'get_post': {
                ...
                const repo = await this.getSocialRepo(username);
                if (!repo)
                    throw new Error('No social repository');
                const issueRes = await this.githubClient.get(`/repos/${encodeURIComponent(username)}/${encodeURIComponent(SOCIAL_REPO)}/issues?state=all&sort=created&direction=desc`);
                const issue = issueRes.data?.find((i) => i.title && i.title.includes(postId));
                if (!issue)
                    throw new Error('Post not found');
                return { issueNumber: issue.number, title: issue.title, body: issue.body };
            }
            case 'get_feed': {
            ...
            case 'get_social_graph': {
                ...
                const [followersRes, followingRes] = await Promise.all([
                    this.githubClient.get(`/users/${encodeURIComponent(username)}/followers?per_page=100`, { auth: false }),
                    this.githubClient.get(`/users/${encodeURIComponent(username)}/following?per_page=100`, { auth: false }),
                ]);
                return {
            ...
            case 'create_post': {
                ...
                const { content } = args;
                const me = await this.githubClient.get('/user');
                let repo = await this.getSocialRepo(me.login);
                if (!repo) {
                    repo = await this.githubClient.post('/user/repos', {
                        name: SOCIAL_REPO,
                ...
                await this.githubClient.post(`/repos/${encodeURIComponent(me.login)}/${encodeURIComponent(SOCIAL_REPO)}/contents/${encodeURIComponent(path)}`, {
                    message: `Add post ${id}`,
            ...
            case 'follow': {
                if (!this.token)
                    throw new Error('Authentication required');
                const { target } = args;
                await this.githubClient.put(`/user/following/${encodeURIComponent(target)}`, null, { auth: true });
            ...
            case 'unfollow': {
                if (!this.token)
                    throw new Error('Authentication required');
                const { target } = args;
                await this.githubClient.delete(`/user/following/${encodeURIComponent(target)}`, { auth: true });
...
export async function runMcpServer(token = null) {
