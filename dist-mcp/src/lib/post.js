...
export function postPathFor(id, date) {
...
export function parsePostFile(raw, path) {
    const { data, body } = parseFrontmatter(raw);
    const id = data['id'];
    const author = data['author'];
    const createdAt = data['createdAt'];
    if (!id || !isValidUlid(id))
        throw new Error(`Post file missing valid id: ${path}`);
    if (!author)
        throw new Error(`Post file missing author: ${path}`);
...
export function markdownToText(markdown, maxLength = 140) {
    const text = markdown
        .replace(/!\[([^\]]*)\]\([^)]*\)/g, '$1')
        .replace(/\[([^\]]*)\]\([^)]*\)/g, '$1')
        .replace(/```[\s\S]*?```/g, ' ')
